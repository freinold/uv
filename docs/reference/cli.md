...
</dd><dt><code>--skip-existing</code> <i>skip-existing</i></dt><dd><p>Don&#8217;t upload a file if it already exists on the index. The value is the URL of the index.</p>
...
<p>The index must provide one of the supported hashes (SHA-256, TODO) for the uploaded files.</p>

<p>May also be set with the <code>UV_PUBLISH_SKIP_EXISTING</code> environment variable.</p>
